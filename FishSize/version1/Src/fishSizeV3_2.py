...
from Lib.depth_anything_v2.dpt import DepthAnythingV2
...
class FishSizeBase(metaclass=ABCMeta):
    ...
    def __init__(self):
        ...
        self.YOLOV8 = YOLO("./Model/Custom/halibut_nano.pt")  # YOLO model path
        self.CORR = 600  # Correction factor for converting pixel to cm

        # Initialize fish species enumeration / 물고기 종에 대한 열거형 선언
        self.Fish = Enum(
            "Fish",
            ["Olive-Flounder", "Olive-Ground"],
            start=0,
        )

        # Model configurations for depth estimation / 깊이 추정 모델 설정
        self.model_configs = {
            "vits": {
                "encoder": "vits",
                "features": 64,
        ...
        self.DEPTHANYTHING = (
            DepthAnythingV2(**self.model_configs["vits"]).to(self.DEVICE).eval()
        )
        self.DEPTHANYTHING.load_state_dict(
            torch.load(
                f'./Model/Custom/depth_anything_v2_vits.pth',
                map_location="cpu",
            )
        )
    ...
    def checkInputPath(self, path: str) -> List[Tuple[str, Union[str, Path]]]:
        ...
        if path_obj.is_file():
            ...
            result.append((mime_type, str(path_obj)))
    ...
    def createUniqueDirectory(sel, project_path: str) -> Path:
    ...
    def checkFileType(
        self, file_path: Union[Path, str], file_type: str
        ...
        return basename, extension, flag
    ...
    def drawLine(self, image: np.ndarray, keypoints: List[Tuple[float, float]]) -> None:
        ...
        if self.checkZeroKeypoints(kp3) and self.checkZeroKeypoints(kp4):
            # Draw lines between specific keypoints / 특정 키포인트 사이에 선 그리기
            cv2.line(
                image, (list(map(int, kp3))), (list(map(int, kp4))), (255, 0, 0), 5
            )
...
class FishSizeV3_2(FishSizeBase):
    ...
    def predict(self):
        ...
        if self.save or self.save_txt:
            ...
            self.output_path = self.createUniqueDirectory(self.project)
            self.csv_path = Path(self.output_path) / "output.csv"

        for file_type, file in tqdm(
            PATH, desc="Progress - ", position=0, leave=self.verbose
        ):
            self.basename, self.extension, self.flag = self.checkFileType(
                file, file_type
            )
            ...
            if self.flag:
                ...
                depth_img = self.DEPTHANYTHING.infer_image(img)
            ...
            # Process video files / 비디오 파일 처리
                ...
                while cap.isOpened():
                    ...
                    depth_frame = self.DEPTHANYTHING.infer_image(frame)
        ...
        if self.save or self.save_txt:
            print(
                f"\nOUTPUT: {self.output_path}/{self.basename}_pred{self.extension}\n"
            )
